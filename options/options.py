...
def set_data_arguments(parser):
    ...
    data_arg.add_argument('--dataset_name', choices=['nba'],
                          type=str, default='nba', help='dataset name')
    ...
    data_arg.add_argument('--template_path', type=str,
                          default='./data/nba_template.png', help='playfield template image')
...
def set_warp_arguments(parser):
    ...
    warp_arg.add_argument('--warp_type', choices=['homography'], type=str, default='homography',
                          help='how to warp images')
    warp_arg.add_argument('--homo_param_method', choices=['deep_homography'], type=str, default='deep_homography',
                          help='how to parameterize homography')


def set_dataset_paths(opt, global_config):
    if opt.dataset_name == 'nba':
        opt.root_dir_path = global_config[opt.dataset_name]['dataset']
        # if opt.mode == 'test':
        opt.test_dataset_path = os.path.join(global_config[opt.dataset_name]['dataset'], 'test')
    else:
        raise ValueError('unknown dataset_name: {0}'.format(opt.dataset_name))
    return opt


def set_init_guesser_options():
    ...
    parser.add_argument('--train_epochs', type=int, default=15, help='number of epochs for training')
